...
var index_default = defineDkgPlugin((ctx, mcp, api) => {
  mcp.registerTool(
    "parallelpedia-get-community-note",
    {
      ...
      inputSchema: {
        topicId: z.string().describe("Topic identifier (e.g., 'Climate_change', 'Artificial_intelligence')")
      }
    },
    async ({ topicId }) => {
      try {
  ...
  mcp.registerTool(
    "parallelpedia-search-community-notes",
    {
      ...
      inputSchema: {
        keyword: z.string().optional().describe("Search keyword to match against topic IDs or titles"),
        minTrustScore: z.number().optional().describe("Minimum trust score (0-100)"),
        maxTrustScore: z.number().optional().describe("Maximum trust score (0-100)"),
        ...
        try {
          ...
          try {
            ...
            return res.status(404).json({
              topicId,
              found: false,
              error: errorMessage.includes("500") ? "SPARQL query failed: Remote OT-Node returned 500 error. Remote testnet nodes may not support SPARQL queries immediately. Try retrieving the asset by UAL instead: GET /api/dkg/assets?ual=YOUR_UAL" : "SPARQL query failed. The data may not be indexed yet, or the query syntax may need adjustment."
            });
          }
          if (!queryResult || !queryResult.data || queryResult.data.length === 0) {
            return res.status(404).json({
    ...
    openAPIRoute(
      {
        ...
        query: z.object({
          ...
          minTrustScore: z.number({ coerce: true }).optional().openapi({ description: "Minimum trust score (0-100)" }),
          maxTrustScore: z.number({ coerce: true }).optional().openapi({ description: "Maximum trust score (0-100)" }),
          limit: z.number({ coerce: true }).optional().default(10).openapi({ description: "Maximum results" })
        }),
        response: {
    ...
    openAPIRoute(
      {
        ...
        body: z.object({
          ...
          trustScore: z.number().min(0).max(100).openapi({ description: "Trust score (0-100)" }),
          summary: z.string().openapi({ description: "Summary of findings" }),
          labelsCount: z.record(z.string(), z.number()).openapi({ description: "Count of each label type" }),
          keyExamples: z.array(
            z.object({
              text: z.string(),
              label: z.string()
